...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Convert a movie to GIF with optional subtitles"
    )
    ...
    parser.add_argument(
        ...
        dest="subtitles",
        action="store",
        help="file containing subtitles",
    )
    parser.add_argument(
        "-t, --timestamp",
        dest="timestamp",
        action="store",
        help="timestamp to start GIF from in HH:MM:SS format",
    )
    parser.add_argument(
        "-d, --duration",
        dest="duration",
        action="store",
        help="length of GIF in seconds",
    )
    ...
    if args.subtitles is not None and not os.path.exists(args.subtitles):
    ...
    if not args.dry_run:
        try:
            subprocess.run(process_args, shell=True, cwd=os.getcwd())
